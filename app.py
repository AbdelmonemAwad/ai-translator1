...
def settings_media(subsection='overview'):
    ...
    if request.method == 'POST':
        ...
        log_to_db("DEBUG", f"Received POST data for media settings: {dict(request.form)}")
        ...
        for key, value in request.form.items():
            if key.startswith('_'):
                continue
            try:
                # Save setting and ensure it's committed
                update_setting(key, value)
                ...
                if saved_value == value:
                    saved_count += 1
                    log_to_db("DEBUG", f"Successfully saved setting: {key} = {value}")
                else:
                    log_to_db("WARNING", f"Setting not saved correctly: {key} = {value} (got: {saved_value})")
                    
            except Exception as e:
                log_to_db("ERROR", f"Failed to save setting {key}: {str(e)}")
        
        # Force database commit
        try:
            db.session.commit()
        except Exception as e:
            log_to_db("ERROR", f"Database commit failed: {str(e)}")
        
        # Provide detailed feedback
        if saved_count > 0:
            flash(f'تم حفظ {saved_count} إعدادات خوادم الوسائط بنجاح', 'success')
            log_to_db("INFO", f"Media settings updated - {subsection} - {saved_count} settings saved")
        else:
            flash('لم يتم حفظ أي إعدادات', 'warning')
            log_to_db("WARNING", "No media settings were saved")
        
        # Redirect to refresh the page with updated values
    ...
    current_settings = {}
    # Include all media-related sections and API section
    media_sections = ['PLEX', 'JELLYFIN', 'EMBY', 'KODI', 'RADARR', 'SONARR', 'MEDIA_SERVICES', 'API']
    for setting in Settings.query.filter(Settings.section.in_(media_sections)).all():
        current_settings[setting.key] = setting.value
    
    # Also include specific media service settings that might be in different sections
    media_keys = [
        'plex_url', 'plex_token', 'plex_enabled',
        'jellyfin_url', 'jellyfin_api_key', 'jellyfin_enabled', 
        'emby_url', 'emby_api_key', 'emby_enabled',
        'kodi_url', 'kodi_username', 'kodi_password', 'kodi_enabled',
        'radarr_url', 'radarr_api_key', 'radarr_enabled',
        'sonarr_url', 'sonarr_api_key', 'sonarr_enabled'
    ]
    
    # Get individual settings to ensure they're included
    for key in media_keys:
        if key not in current_settings:
            setting_value = get_setting(key, '')
            current_settings[key] = setting_value
            log_to_db("DEBUG", f"Loaded media setting: {key} = {setting_value}")
    
    log_to_db("DEBUG", f"Total media settings loaded: {len(current_settings)}")
