#!/usr/bin/env python3
"""
Applying fix for PyTorch version check and improved error handling.
"""
"""
...
class PyTorchTextProcessor:
    ...
    def _check_pytorch_availability(self) -> bool:
        """Check if PyTorch is available and compatible"""
        try:
            import torch
            ...
            # Updated version check for PyTorch 2.1+
            if major < 2:
                logger.warning(f"PyTorch {torch_version} found, but >= 2.1 recommended")
                return False
            elif major >= 2 and minor >= 1:
                logger.info(f"PyTorch {torch_version} available (optimal version)")
                return True
            elif major == 2 and minor == 0:
                logger.info(f"PyTorch {torch_version} available (compatible but upgrade recommended)")
                return True
            else:
            ...
            logger.warning(f"PyTorch check failed: {str(e)}")
            return False
...
# This code modifies the _check_pytorch_availability function to support newer versions of PyTorch (>= 2.1).
class PyTorchTextProcessor:
    """معالج النصوص باستخدام PyTorch"""
