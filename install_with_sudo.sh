...
# AI Translator v2.2.4 Installation Script with sudo handling
...
# Install required packages with password
echo "📦 Installing system packages..."
echo '1q1' | sudo -S DEBIAN_FRONTEND=noninteractive apt update -y
echo '1q1' | sudo -S DEBIAN_FRONTEND=noninteractive apt install -y \
    wget curl git unzip \
...
cd /home/eg2
echo '1q1' | sudo -S systemctl stop ai-translator 2>/dev/null || true
echo '1q1' | sudo -S rm -rf ai-translator 2>/dev/null || true
...
echo '1q1' | sudo -S systemctl start postgresql
echo '1q1' | sudo -S systemctl enable postgresql

echo '1q1' | sudo -S -u postgres psql -c "DROP DATABASE IF EXISTS ai_translator;" 2>/dev/null || true
echo '1q1' | sudo -S -u postgres psql -c "DROP USER IF EXISTS ai_translator;" 2>/dev/null || true
echo '1q1' | sudo -S -u postgres psql -c "CREATE USER ai_translator WITH PASSWORD 'ai_translator_pass2024';"
echo '1q1' | sudo -S -u postgres psql -c "CREATE DATABASE ai_translator OWNER ai_translator;"
...
echo '1q1' | sudo -S tee /etc/systemd/system/ai-translator.service > /dev/null << 'SERVICE'
[Unit]
...
echo '1q1' | sudo -S tee /etc/nginx/sites-available/ai-translator > /dev/null << 'NGINX'
server {
    listen 80;
    server_name _;
...
echo '1q1' | sudo -S ln -sf /etc/nginx/sites-available/ai-translator /etc/nginx/sites-enabled/
echo '1q1' | sudo -S rm -f /etc/nginx/sites-enabled/default

if echo '1q1' | sudo -S nginx -t; then
...
echo '1q1' | sudo -S chown -R eg2:eg2 /home/eg2/ai-translator
chmod 600 .env

# Start services
echo "🚀 Starting services..."
echo '1q1' | sudo -S systemctl daemon-reload
echo '1q1' | sudo -S systemctl enable ai-translator
echo '1q1' | sudo -S systemctl restart postgresql
echo '1q1' | sudo -S systemctl restart nginx
echo '1q1' | sudo -S systemctl start ai-translator
...
for service in postgresql nginx ai-translator; do
    if systemctl is-active --quiet $service; then
        ...
        echo '1q1' | sudo -S journalctl -u $service --no-pager -n 3 | sed 's/^/  /'
    fi
done
